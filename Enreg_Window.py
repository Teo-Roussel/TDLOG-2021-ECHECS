...
class Ui_SaveWindow(object):
    def __init__(self,MainWindow,PVP, InitialWindowAssoc = None, playWindowAssoc = None, FEN=None):
        ...
        self.label.setStyleSheet("border-color : rgb(80,0,0);\n border-width : 1px; \n border-style : inset;")
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("Images_IG/bouton_valider.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def SaveMethod(self):
        ...
        if stringName.strip(" ") != "":
            if self.PVP:
                ...
                namePlayer = self.windowAssoc.textW if self.windowAssoc.colorIA == 1 else self.windowAssoc.textB
            realName = ("Save/AI_mode_{}_{}_".format(coulIA,namePlayer) if not self.PVP else "Save/PVP_mode_{}".format(totNames))+stringName+".txt"
            f = open(realName, "w")
    ...
    def OpenMethod(self):
        stringName = self.saveEdit.text()
        ListFiles = os.listdir("Save")
        realName = ("PVP_mode_" if self.PVP else "AI_mode_")+stringName+("" if stringName[-4:]==".txt" else ".txt")
        if realName in ListFiles:
            ...
            if self.PVP:
                ...
                while realName[indFinNamePlayer] != '_':
                    indFinNamePlayer += 1
                self.windowAssoc.textW = "AI" if stringName[0] == "B" else realName[10:indFinNamePlayer]
                self.windowAssoc.textB = "AI" if stringName[0] == "W" else realName[10:indFinNamePlayer]
            ...
            self.textlabel = "Error - " + (" Save as :" if self.FEN != None else " Open : ")
            self.saveEdit.clear()
